...
export const uploadFormDataController = async (request, response) => {
    try {
        ...
        var arr1 = [];
        var arr2 = [];
        for(var i=0;i<fileObj['profile'].length;i++){
            arr1.push(fileObj['profile'][i].filename);
        }
        for(var i=0;i<fileObj['profileOne'].length;i++){
            arr2.push(fileObj['profileOne'][i].filename);
        }

        request.body._id = uuid();
        request.body.email = request.session.email;
        request.body.profile = arr1;
        request.body.profileOne = arr2;
